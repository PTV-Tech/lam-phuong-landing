...
export async function GET(request: NextRequest) {
  ...
  const url = `https://api.airtable.com/v0/${process.env.BASE_ID}/${process.env.JOBS_TABLE}?pageSize=${PAGE_SIZE}&${fields}&offset=${offset}`;
  ...
  return new Response(JSON.stringify({ jobs, offset: response.offset }), {
    status: 200,
